...
__all__ = ['QuantileLoss', 'pretrain_1d', 'train_1d', 'TrainResult']
...
class QuantileLoss:
    ...
    def __call__(self, input, target, cdfs_pretrain=None, p0=1., p0_weights=None,
                 p0_replacement=True):
        ...
        if cdfs_pretrain is not None:
            if cdfs_pretrain == 'central':
                results_raw = results_raw[..., (self.cdfs_pred.shape[-1] - 1) // 2]
            else:
                results_raw = results_raw[..., cdfs_pretrain]
        elif not (p0 == 1. and p0_weights is None):
...
def pretrain_1d(quantile_net_1d, device='cpu', cdfs_pretrain='central',
                save_path=None, save_period=1,
    ...
    if isinstance(quantile_net_1d, _QuantileInterp1D):
        warnings.warn('QuantileInterp1D does not require pretrain.', RuntimeWarning)
        return PretrainResult(last_state=None, l0_train=[], l0_valid=[])

    elif isinstance(quantile_net_1d, QuantileNet1D) or isinstance(quantile_net_1d, DDP):
        model, model_local = _check_model(quantile_net_1d, device)
        verbose = _check_verbose(verbose, 1)
        optimizer_kwargs = {} if optimizer_kwargs is None else optimizer_kwargs
        train_loader, valid_loader = _get_data_loader(x, theta, batch_size, validation_fraction,
                                                      train_loader, valid_loader)
        if rescale_data:
            _rescale_data(model_local, train_loader, device)

        loss = QuantileLoss(model_local.cdfs_pred, device=device)
        optimizer = _check_optimizer(optimizer, learning_rate, optimizer_kwargs, model)

        prev_state = _get_prev_state(save_path, device)
        if prev_state is None:
            if (not dist.is_initialized()) or dist.get_rank() == 0:
                if verbose > 0:
                    print(f'[{datetime.datetime.now()}]  did not find previous checkpoint, will '
                          f'pretrain from scratch.', flush=True)
            l0_train = []
            l0_valid = []
        elif isinstance(prev_state, PretrainResult):
            if (not dist.is_initialized()) or dist.get_rank() == 0:
                if verbose > 0:
                    print(f'[{datetime.datetime.now()}]  found previous pretraining checkpoint, '
                          f'will resume pretraining from that.', flush=True)
        ...
        elif isinstance(prev_state, TrainResult):
            if (not dist.is_initialized()) or dist.get_rank() == 0:
                if verbose > 0:
                    print(f'[{datetime.datetime.now()}]  skipping pretraining as save_path is '
                          f'already a TrainResult.', flush=True)
            return prev_state
        else:
            raise RuntimeError('Unexpected data type for prev_state.')

        if dist.is_initialized():
            dist.barrier()

        while len(l0_train) < max_epochs:
            i_epoch = len(l0_train)
            if dist.is_initialized():
            ...
            for j, batch_now in enumerate(train_loader):
                ...
                l0_now = loss(y_now[0], theta_now[..., model_local.i], cdfs_pretrain=cdfs_pretrain)
                optimizer.zero_grad()
                l0_now.backward()
                optimizer.step()
                l0_train_now += l0_now.detach() * theta_now.shape[0]
                n_now += theta_now.shape[0]
            l0_train_now = _compute_total_loss(l0_train_now, None, n_now)
            ...
            with torch.no_grad():
                for j, batch_now in enumerate(valid_loader):
                    ...
                    l0_now = loss(y_now[0], theta_now[..., model_local.i],
                                  cdfs_pretrain=cdfs_pretrain)
                    l0_valid_now += l0_now.detach() * theta_now.shape[0]
                    n_now += theta_now.shape[0]
            l0_valid_now = _compute_total_loss(l0_valid_now, None, n_now)
            ...
            if (not dist.is_initialized()) or dist.get_rank() == 0:
                if verbose > 0 and (i_epoch + 1) % verbose == 0:
                    print(f'[{datetime.datetime.now()}]  finished pretraining epoch {i_epoch + 1}, '
                          f'l0_train = {l0_train_now:.5f}, l0_valid = {l0_valid_now:.5f}',
                          flush=True)
                if save_path is not None:
                    if save_period <= 0 or (i_epoch + 1) % save_period == 0:
                        ...
                        torch.save(PretrainResult(last_state=last_state,
                                                  l0_train=np.asarray(l0_train),
                                                  l0_valid=np.asarray(l0_valid))._asdict(),
        ...
        train_result = PretrainResult(last_state=last_state,
                                      l0_train=np.asarray(l0_train),
        ...
        if (not dist.is_initialized()) or dist.get_rank() == 0:
            if save_path is not None:
                ...
                os.rename(save_path + '.tmp', save_path)
            if verbose > 0:
                print(f'[{datetime.datetime.now()}]  finished pretraining dim {model_local.i}, '
                      f'l0_valid = {l0_valid[-1]:.5f}', flush=True)
        return train_result

    else:
        raise ValueError("I don't know how to pretrain this quantile_net_1d.")
...
def train_1d(quantile_net_1d, device='cpu', save_path=None, save_period=5,
    ...
    if isinstance(quantile_net_1d, _QuantileInterp1D): # for the first dim without x, no nn required
        ...
        return TrainResult(best_state=quantile_net_1d.configs, last_state=quantile_net_1d.configs,
                           lambda_reg=None, l0_train=[], l1_train=[], l0_valid=[], l1_valid=[])

    elif isinstance(quantile_net_1d, QuantileNet1D) or isinstance(quantile_net_1d, DDP):
        model, model_local = _check_model(quantile_net_1d, device)
        verbose = _check_verbose(verbose, 5)
        optimizer_kwargs = {} if optimizer_kwargs is None else optimizer_kwargs
        ...
        optimizer = _check_optimizer(optimizer, learning_rate, optimizer_kwargs, model)
        scheduler = _check_scheduler(scheduler, learning_rate_decay_delay,
                                     learning_rate_decay_period, learning_rate_decay_gamma,
        ...
        if prev_state is None:
            if (not dist.is_initialized()) or dist.get_rank() == 0:
                if verbose > 0:
                    print(f'[{datetime.datetime.now()}]  did not find previous checkpoint, will '
                          f'train from scratch.', flush=True)
            l0_train = []
            l1_train = []
            l0_valid = []
            l1_valid = []
            ...
            if (not dist.is_initialized()) or dist.get_rank() == 0:
                if verbose > 0:
                    print(f'[{datetime.datetime.now()}]  found previous pretraining checkpoint, '
                          f'will start training from that.', flush=True)
            model_local.load_state_dict(prev_state.last_state.model)
            l0_train = []
            l1_train = []
            l0_valid = []
            ...
            if (not dist.is_initialized()) or dist.get_rank() == 0:
                if verbose > 0:
                    print(f'[{datetime.datetime.now()}]  found previous training checkpoint, will '
                          f'resume training from that.', flush=True)
            model_local.load_state_dict(prev_state.last_state.model)
        ...
        while not _check_convergence(l0_valid, l1_valid, lambda_reg, stop_after_epochs, stop_tol,
            ...
            if (not dist.is_initialized()) or dist.get_rank() == 0:
                if verbose > 0 and (i_epoch + 1) % verbose == 0:
                    print(f'[{datetime.datetime.now()}]  finished training epoch {i_epoch + 1}, '
        ...
        if return_best_epoch:
            model_local.load_state_dict({k: v.to(device) for k, v in best_state.model.items()})
...
PretrainResult = namedtuple('PretrainResult', ['last_state', 'l0_train', 'l0_valid'])


FullState = namedtuple('FullState', ['model', 'optimizer', 'scheduler'])


def _check_model(quantile_net_1d, device):
...
def _check_verbose(verbose, default):
    if verbose is True:
        verbose = default
...
def _get_data_loader(x, theta, batch_size, validation_fraction, train_loader, valid_loader):
    if theta is not None:
        if dist.is_initialized():
...
def _rescale_data(model_local, train_loader, device):
    if dist.is_initialized():
    ...
    for batch_now in train_loader:
        ...
        if model_local.i > 0:
            mu_theta.append(torch.mean(theta_now[..., :model_local.i], dim=0, keepdim=True))
            sigma_theta.append(torch.std(theta_now[..., :model_local.i], dim=0, keepdim=True))
...
def _check_optimizer(optimizer, learning_rate, optimizer_kwargs, model):
...
def _check_scheduler(scheduler, learning_rate_decay_delay, learning_rate_decay_period,
                     learning_rate_decay_gamma, scheduler_kwargs, optimizer):
    if isinstance(scheduler, type) and issubclass(scheduler,
        ...
        scheduler = eval('torch.optim.lr_scheduler.' + scheduler)
        scheduler = scheduler(optimizer, **scheduler_kwargs)
    else:
        raise ValueError
    return scheduler
...
def _get_prev_state(save_path, device):
    if save_path is None or not os.path.isfile(save_path):
        return None
    else:
        prev_state = torch.load(save_path, map_location=device)
        if 'lambda_reg' in prev_state:
            return TrainResult(**prev_state)
        else:
            return PretrainResult(**prev_state)
